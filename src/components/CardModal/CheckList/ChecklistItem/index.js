...
const ChecklistItem = ({ checklist }) => {
  ...
  const handleCheckListItemAdd = () => {
    if (itemName == "") {
      alert("You have to type an item name");
    } else {
    ...
    <>
      ...
      >
        <Box>
          <Typography>
            {" "}
            {totalChecklistItem()}/{checklist.items.length}{" "}
          </Typography>
        </Box>
        <Box>
          {" "}
          <LinearProgress
            ...
            variant="determinate"
            value={
              totalChecklistItem() === 0 && checklist.items.length === 0
                ? 0
                : (totalChecklistItem() / checklist.items.length) * 100
            }
          />
        </Box>
      </Box>
        ...
        checklist.items.map((item) => (
          ...
          >
            ...
            <TextField
              ...
              onChange={(event) =>
                handleCheckListItemEdit(event.currentTarget.value, item.id)
              }
            />
